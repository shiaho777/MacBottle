...
#endif
int main(void){
    uint64_t x=1, best=1e9;
    volatile uint64_t sink=0;
    double tb=1e9;
    for(int r=0;r<5;r++){
        double t0=now_sec();
        for(uint64_t i=1;i<=200000000ull;i++) x = x*6364136223846793005ull+1442695040888963407ull;
        double dt=now_sec()-t0;
        if(dt<tb) tb=dt;
    }
    sink=x;
    printf("pure-reg-loop best: %.4f s (x=%llu)\n", tb, (unsigned long long)sink);
    return 0;
}
