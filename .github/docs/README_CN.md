...
2025 年 5 月,Whisky 宣布停止维护,13k+ 用户失去了 macOS 上最好的 Wine 封装工具,也留下了无处安放的需求。MacBottle 接下了这个位置——并且恰恰在那些最难的地方,与所有其他方案拉开差距:

- **我们是接续者,不是克隆。** MacBottle 是 [Whisky](https://github.com/Whisky-App/Whisky)(Isaac Marovitz 创建,GPL-3.0)的社区驱动 fork。所有原作者的署名被完整保留;而维护、修 bug、加功能,由社区继续。
- **永远不需要命令行。** 其他方案让你手搓 wine prefix、折腾 `WINEPREFIX`、背环境变量。MacBottle 用原生 macOS 界面完成瓶子的创建、配置与启动——终端可有可无,而不是必须。
- **免费,且开源。** CrossOver 为同样的便利收取授权费。MacBottle 在 GPL-3.0 下提供同类体验,用贡献而不是订阅来支付。
- **配方系统是护城河。** 一份配方就是一份 JSON 文件,描述「让这款游戏跑起来需要什么」:环境变量、winetricks、注册表调整、渲染器选择。社区验证、远程同步、启动时自动应用。大多数封装工具止步于「游戏也许能跑」,MacBottle 学会的是「这款游戏该怎么跑」。
- **为每次启动选择正确的引擎。** 每次启动都会扫描可执行文件(PE 导入 + 运行时画像),结合挂载的配方,自动决定用 **D3DMetal**、**DXVK** 还是 **WineD3D**,并切换到匹配的 Wine 引擎——不用你操心。
- **中文与英文是一等公民。** 界面、配方、文档从第一天起就双语维护——而不是事后才补的翻译。
