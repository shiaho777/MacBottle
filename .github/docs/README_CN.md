...
2025 年 5 月,macOS 上最好的 Wine 封装工具 [Whisky](https://github.com/Whisky-App/Whisky) 宣布停止维护,超过一万三千名用户一夜之间失去了维护者。MacBottle 就是接手这件事的社区,而它在普通封装工具做不到的地方拉开差距:

- **持续维护中。** MacBottle 是 Whisky(Isaac Marovitz 创建,GPL-3.0)的社区接续版本。原作者的全部署名完整保留;后续的 bug 修复和新功能由社区持续推进。
- **不需要命令行。** 不用手搓 wine prefix,不用背 `WINEPREFIX`。MacBottle 用原生 macOS 窗口完成每一个瓶子的创建、配置与启动。
- **免费,且开源。** CrossOver 为这种便利收取授权费,MacBottle 在 GPL-3.0 下提供同等的体验——用贡献来支付,而不是订阅。
