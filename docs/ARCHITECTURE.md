...
- **Meta updates only on partial success.** If the user selects 5 changes
  and 3 download but 2 fail, the 3 are cached and the meta records the full
  new index so the remaining 2 reappear on the next check. Fully-failed
  applies leave the previous state intact.
